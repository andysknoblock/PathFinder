...
public class DankPaths {
	...
	public static void main(String[] args) {
		...
		Grid g = new Grid();
		frame.add(g);
		System.out.println("Weclome to the Path Counter\nThis application counts the number of possible paths from the upper right corner of the screen to the bottom left corner by moving only right and down");
		...
		else 
		{
			...
			System.out.println("Enter the number of columns in your grid:");
			if (input.hasNextInt()) {
				cols = input.nextInt();
			} 
			else {
			...
			if (!fail) {
				...
				g.printGrid();
				System.out.println("Number of total paths: "+ g.countPathsRightDown(0, 0));
			} else {
